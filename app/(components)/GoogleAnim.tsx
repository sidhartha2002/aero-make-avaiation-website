...
const Card = ({ className, title }: { className?: string; title?: string }) => (
  ...
  >
    ...
    <div className="pointer-events-none absolute flex h-full w-full flex-col items-start rounded-3xl bg-gray-100 p-8 shadow-2xl [grid-area:1/1]">
      ...
      {title && (
        <p className="font-bold leading-tight text-5xl md:text-6xl">{title}</p>
      )}
...
const ActionButton = ({
  ...
  <button
    onClick={onClick}
    className={twMerge(
      " p-96 relative flex w-full items-center justify-center font-bold text-[rgba(0,0,0,.6)] transition-[transform,color] duration-500 focus-visible:text-white group-hover:text-white md:text-4xl",
      direction === "right" &&
